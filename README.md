...
ARIA - локальная языковая модель на основе LSTM с обучением через обратную связь (RLHF-like), написанная на Rust. Вычисления выполняются на NVIDIA GPU через CUDA/cuBLAS с кастомными PTX ядрами.
