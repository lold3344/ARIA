...
> Текущая версия: **3.2**. Следующая планируемая версия: **ARIA Wotan 3.3**.
...
Чем больше чистого и разнообразного текста, тем лучше качество. Рекомендуемый минимум: **500-1000 МБ** суммарно.

При первом запуске токенизатор строит BPE-словарь на основе диалоговых данных:

![BPE обучение](screenshots/BPE-Learn.png)
...
**2. Диалоговое обучение (SFT)** - при обучении на формате `Пользователь: ...\nАссистент: ...` модель видит ролевые токены `<USER>` / `<ASSISTANT>` и обучается только на токенах ассистента. Пример кодировки диалога с маской:

![Маска диалога](screenshots/Token-Dialog-Mask-Check.png)

**3. Дообучение через фидбек (RLHF)** - происходит в реальном времени во время диалога. После каждого ответа ARIA спрашивает оценку:
